...
function run(p) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        const ps = child_process_1.exec(command, function (err, stdout, stderr) {
            if (err != null) {
...
async function main(p) {
    ...
    await trycatch('main', async (log) => {
        ...
        exports.koThrottler.subscribe((v) => {
            const n = parseInt(v);
            if (n > 0)
                exports.koThrottler(n);
            else
                exports.koThrottler(10);
        });
        exports.koSelectAll.subscribe((v) => {
            for (const entry of exports.koSnapshots().entries())
                entry.checked(v);
        });
...
class Snapshots {
    ...
    getFooters() {
        ...
        const strRuns = self.runs().map((runsList) => {
            ...
            return exports.koHumanReadable() ? common.humanFileSize(total) : ('' + total);
        });
...
async function clickRefresh() {
    await trycatch('refresh', async (log) => {
        exports.koSnapshots(null);
        btrfs.config.useSudo = exports.koUseSudo();
        const { list } = await btrfs.listSnapshots({ log: log.child('list'),
            name: exports.koName(),
            dir: exports.koDirectory(),
        });
        if (exports.koReverse())
            list.reverse();
        const snapshots = new Snapshots({ entries: list });
        exports.koSnapshots(snapshots);
    });
}
exports.clickRefresh = clickRefresh;
async function clickLaunch() {
    const n = exports.koSnapshots().runs().length + 1;
    await trycatch(`run_${n}`, async (log) => {
        log.log('Throttler:', exports.koThrottler());
        const throttler = new common.TasksThrotther(exports.koThrottler());
        const entries = exports.koSnapshots().entries().filter((entry) => entry.checked());
        ...
        log.log(`Append runs`);
        exports.koSnapshots().runs.push(runs);
